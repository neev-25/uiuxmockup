...
export async function POST(req: NextRequest) {
  ...
  if (!projectId || !screenId || !userInput?.trim()) {
    return NextResponse.json(
      { error: "projectId, screenId, and userInput are required" },
      { status: 400 }
    );
  }

  const editPrompt = `
${truncateText(oldCode ?? "", 6000)}

Apply ONLY these changes, keep design/style the same:
${truncateText(userInput, 1000)}
  `.trim();
